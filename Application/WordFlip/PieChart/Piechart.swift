    ...
    public override func drawRect(rect: CGRect) {
        ...
        for (index, slice) in slices.enumerate() {
            ...
            if (index == activeSlice) {
                color = slice.color
                subtitle = delegate?.setSubtitleForPieChart(self, total:self.total, slice: slice) ?? "subtitle"
                info = delegate?.setInfoForPieChart(self, total: self.total, slice: slice) ?? "info"
            }
            color.setFill()
            path.fill()
